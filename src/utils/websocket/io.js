...
class WebSocketIO extends Event {
  ...
  // reconnectAttempt: 是否是重新尝试打开的，true:是，false：指的是第一次打开
...
function attachEvent(ws, io, reconnectAttempt) {
  ...
  io.onclose = function (e) {
    ...
    if (ws.forcedClose) {
      ...
      const timeout = ws.reconnectInterval * Math.pow(ws.reconnectDecay, ws.reconnectAttempts)
      setTimeout(
        function () {
  ...
  io.onmessage = function (e) {
    ...
    if (typeof e.data === 'string') {
      try {
        ...
        log('onmessage: 解析失败，接受的数据不是json格式')
      }
    }
  }
